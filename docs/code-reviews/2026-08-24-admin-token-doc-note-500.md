...
distinct secret because the server didn't support it yet. Docs-only.
...
- `README.md`: added a `MARKETPLACE_ADMIN_TOKEN` row to the environment
  variables table, and a note in the CI-pipeline section.
- No script logic changed — `bash -n` clean, diff is comment/prose only.
